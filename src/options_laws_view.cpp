...
void options_laws_view::decrease_selected()
{
  switch (m_selected)
  {
...
void options_laws_view::increase_selected()
{
  switch (m_selected)
  {
...
bool options_laws_view::process_event_impl(sf::Event& event)
{
  ...
  else if (event.type == sf::Event::MouseMoved)
  {
    ...
    else if (is_in(mouse_screen_pos, m_layout.get_sound_effects_volume_value())) set_selected(options_laws_view_item::sound_effects_volume);
    else if (is_in(mouse_screen_pos, m_layout.get_starting_pos_value())) set_selected(options_laws_view_item::starting_position);
  }
...
void options_laws_view::process_resize_event_impl(sf::Event& event)
{
  ...
  m_layout = options_laws_view_layout(
    w,
...
void options_laws_view::set_selected(const options_laws_view_item i)
{
  if (m_selected != i)
  {
    game_resources::get().get_sound_effects().play_hide();
...
void options_laws_view::draw_impl()
{
  ...
  m_controls_bar.set_draw_select(
       m_selected == options_laws_view_item::left_controls
    || m_selected == options_laws_view_item::right_controls
  );
  m_controls_bar.draw();
...
void options_laws_view::start_impl()
{
  assert(!is_active());
  set_is_active(true);
}

void options_laws_view::stop_impl()
{
  assert(is_active());
  clear_next_state();
  set_is_active(false);
...
void options_laws_view::tick_impl(const delta_t)
{
  assert(is_active());
  // Nothing here yet
}
